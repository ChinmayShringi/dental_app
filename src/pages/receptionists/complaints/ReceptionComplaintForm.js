...
class ReceptionComplaintForm extends Component {
  api = new Api();
  ...
  getFormData() {
    ...
    if (this.state.callLogId === 0) {
      options.api = 'v_1/receptionists/complaints/add';
    } else {
      options.api = 'v_1/receptionists/complaints/edit';
    }
  ...
  handleChangeOrder = orderId => {
    if (this.state.orderId !== orderId) {
      ...
      if (parseInt(orderId) > 0) {
        ...
        this.api
          .callPostApi({
            api: 'v_1/receptionists/complaints/get-order-details',
            method: 'POST',
            headers: {
              Accept: 'application/json',
              'Content-Type': 'multipart/form-data',
  ...
  handleSubmit = values => {
    ...
    let options = {
      api: 'v_1/receptionists/complaints/store',
      method: 'POST',
      headers: {
        Accept: 'application/json',
      },
      ...
      if (responseData?.status_code === 200) {
        ...
        setTimeout(() => {
          if (this.state.callLogId > 0) {
            ...
            this.props.navigation.navigate('ReceptionComplaints');
          }
        }, 4500);
      } else {
        let errormessage = null;
...
export default connect(mapStateToProps, null)(ReceptionComplaintForm);
