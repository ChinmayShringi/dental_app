...
export default class ReceptionViewOrderDetails extends Component {
      ...
      <View>
        <Card style={callLogCardLayout.cardContainer}>
          <TouchableOpacity
            activeOpacity={0.5}
            onPress={() => {
              this.props.navigation.push('ReceptionViewJob', {
  ...
  getDataURL() {
    ...
    setTimeout(() => {
      this.svg.toDataURL(data => {
        ...
        this.setState({qrcodeimage: shareImageBase64});

        const formData = new FormData();
        formData.append(
          'orderdetailid',
          this.state.orderdetailidforqrcode === null
            ? 0
            : this.state.orderdetailidforqrcode,
        );
        formData.append('qrcodedata', `data:image/png;base64,${data}`);

        let options = {
          api: 'v_1/receptionists/orders/upload-qr-code',
          method: 'POST',
          headers: {
            Accept: 'application/json',
            'Content-Type': 'multipart/form-data',
