...
class ReceptionDeliveryCallForm extends Component {
  api = new Api();
  ...
  getFormData() {
    ...
    if (this.state.callLogId === 0) {
      options.api = 'v_1/receptionists/delivery-calls/add';
    } else {
      options.api = 'v_1/receptionists/delivery-calls/edit';
    }
  ...
  handleChangeOrder = orderId => {
    if (this.state.orderId !== orderId) {
      ...
      if (parseInt(orderId) > 0) {
        ...
        this.api
          .callPostApi({
            api: 'v_1/receptionists/delivery-calls/get-order-details',
            method: 'POST',
            headers: {
              Accept: 'application/json',
              'Content-Type': 'multipart/form-data',
  ...
  handleSubmit = values => {
    ...
    let options = {
      api: 'v_1/receptionists/delivery-calls/store',
      method: 'POST',
      headers: {
        Accept: 'application/json',
      },
      ...
      if (responseData?.status_code === 200) {
        ...
        setTimeout(() => {
          if (this.state.callLogId > 0) {
            ...
            this.props.navigation.navigate('ReceptionDeliveryCalls');
          }
        }, 4500);
      } else {
        let errormessage = null;
...
export default connect(mapStateToProps, null)(ReceptionDeliveryCallForm);
